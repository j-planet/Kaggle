...
def calc_delta_r(x_predicted,y_predicted,x_true,y_true): 
    ...
    distances_perm=np.zeros([num_configurations,num_halos],float) #The distance between eac
                                                                  #true and predicted
                                                                  #halo for every comb
    true_halo_indexes_perm=[] #jjgb log of all the permutations of true halos used
    predicted_halo_indexes_perm=[] #jjgb log of all the predicted permutations
    ...
    for  perm in it.permutations(a[num_halos-2],num_halos):
        ...
        for j in xrange(num_halos): #loop through all the true halos with the
            ...
            which_predicted_halos.append(int(perm[j])) #jjgb log the order in which I true
                                                       #each predicted halo
...
def main(user_fname, fname):
    ...
    r=np.array([],dtype=float) # The array which I will jjgb log all the calculated radial distances
    angle=np.array([],dtype=float) #The array which I will jjgb log all the calculated angles
